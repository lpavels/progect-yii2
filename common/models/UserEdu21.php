...
class UserEdu21 extends ActiveRecord implements IdentityInterface
{
        ...
        if ($program == 1) {
            ...
            if ($lvl == 2) {
                ...
                $command = $connection->createCommand(
                    ...
                    WHERE user.transfer = 2021 and user.training_id = 1 and organization.municipality_id = ' . $id . '
                ...
                $reportTbl_s21 = ReportTbl21::find()->where(['training_id' => 1, 'municipality_id' => $id])->all();
                foreach ($reportTbl_s21 as $reportTbl) {
                ...
                $command = $connection->createCommand(
                    ...
                    FROM organization
                    LEFT JOIN user ON user.organization_id = organization.id
                    WHERE user.transfer = 2021 and user.training_id = 1 AND organization.region_id = ' . $id . '
                ...
                $reportTbl_s21 = ReportTbl21::find()->where(['training_id' => 1, 'region_id' => $id])->all();
                foreach ($reportTbl_s21 as $reportTbl) {
            ...
            } elseif ($lvl == 5) //по федеральному округу
            {
                ...
                $command = $connection->createCommand(
                    ...
                    FROM organization
                    LEFT JOIN user ON user.organization_id = organization.id
                    WHERE user.transfer = 2021 and user.training_id = 1 AND organization.federal_district_id = ' . $id . '
                ...
                $data = [];
                $reportTbl_s21 = ReportTbl21::find()->where(['training_id' => 1, 'federal_district_id' => $id])->all();
                foreach ($reportTbl_s21 as $reportTbl) {
            ...
            if ($lvl == 2) {
                ...
                $command = $connection->createCommand(
                    ...
                    FROM organization
                    LEFT JOIN user ON user.organization_id = organization.id
                    WHERE user.transfer = 2021 and user.training_id = 2 and organization.municipality_id = ' . $id . '
                    GROUP BY user.type_training,organization.id'
                );
                $arr_s = $command->queryAll();
                ...
                $reportTbl_s21 = ReportTbl21::find()->where(['training_id' => 2, 'municipality_id' => $id])->all();
                foreach ($reportTbl_s21 as $reportTbl) {
                    $data[$reportTbl->organization_id . '_inputTest_calc'] += $reportTbl->input_test;
                    $data[$reportTbl->organization_id . '_inputTestCount_calc']++;
                    $data[$reportTbl->organization_id . '_theme1'] += $reportTbl->theme1;
                ...
                $command = $connection->createCommand(
                    ...
                    FROM organization
                    LEFT JOIN user ON user.organization_id = organization.id
                    WHERE user.transfer = 2021 and user.training_id = 2 AND organization.region_id = ' . $id . '
                    GROUP BY user.type_training,organization.municipality_id'
                );
                $arr_s = $command->queryAll();
                foreach ($arr_s as $arr) //кол-во зерегистрированных взрослых и детей в муниципальном районе
                ...
                $reportTbl_s21 = ReportTbl21::find()->where(['training_id' => 2, 'region_id' => $id])->all();
                foreach ($reportTbl_s21 as $reportTbl) {
                    $municipalyty_ids[$reportTbl->municipality_id] = $reportTbl->municipality_id; //ids муниципальных в регионе
                    $organizations_ids[$reportTbl->municipality_id][$reportTbl->organization_id] = $reportTbl->organization_id; //ids оранизаций в мунипальном образовании
            ...
            } elseif ($lvl == 5) //по федеральному округу
            {
                ...
                $command = $connection->createCommand(
                    ...
                    FROM organization
                    LEFT JOIN user ON user.organization_id = organization.id
                    WHERE user.transfer = 2021 and user.training_id = 2 AND organization.federal_district_id = ' . $id . '
                    GROUP BY user.type_training,organization.region_id'
                );
                $arr_s = $command->queryAll();
                ...
                $data = [];
                $reportTbl_s21 = ReportTbl21::find()->where(['training_id' => 2, 'federal_district_id' => $id])->all();
                foreach ($reportTbl_s21 as $reportTbl) {
                    $region_ids[$reportTbl->region_id] = $reportTbl->region_id; //ids регионов в федеральном
                    $municipalyty_ids[$reportTbl->region_id][$reportTbl->municipality_id] = $reportTbl->municipality_id; //ids муниципальных в регионе
                    $organizations_ids[$reportTbl->municipality_id][$reportTbl->organization_id] = $reportTbl->organization_id; //ids оранизаций в мунипальном образовании
