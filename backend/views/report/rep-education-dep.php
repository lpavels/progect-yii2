...
$name_organizations = Organization::find()->where(['municipality_id' => Municipality::findOne($depEdu->municipality_id)->id, 'status'=>1])->orderby(['type_org' => SORT_ASC, 'short_title' => SORT_ASC])->all();
...
elseif (!empty($mun_org)) //по муниципальному
{
    if ($post['title'] == 2022)
    {
        $mun_array = $user_model->reportNew($depEdu->municipality_id, 1, 2);
    }
    elseif ($post['title'] == 2021)
    {
        $mun_array = $user_model21->reportNew($depEdu->municipality_id, 1, 2);
    }
    elseif ($post['title'] == 2020)
    {
        $mun_array = $user_model20->reportNew($depEdu->municipality_id, 1, 2);
    }
