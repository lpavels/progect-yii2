...
if (!empty($post['municipality_id']))
{
    ...
    $name_organizations = Organization::find()->where(['municipality_id' => $post['municipality_id']])->orderBy(['type_org'=>SORT_DESC,'short_title'=>SORT_ASC])->all();
...
$two_column = ['options' => ['class' => 'row mt-4'], 'labelOptions' => ['class' => 'col-2 col-form-label font-weight-bold']];
...
?>
    <div class="report-adm container">
        <div class="row">
            <div class="col-2"></div>
            <div class="col-8">
                ...
                <div class="row">
                    ...
                    <div class="col">
                        <?= $form->field($model, 'title')->dropDownList($year_ar, ['options' => [$post['title'] => ['Selected' => true]]])->label(false); ?>
                    </div>
                </div>
                ...
                <div class="row">
                    ...
                    <div class="col">
                        <?= $form->field($model, 'short_title')->dropDownList($name_organization_items, ['options' => [$post['short_title'] => ['Selected' => true]]])->label(false); ?>
                    </div>
                </div>
    ...
    <table id="tableId" class="table table-hover table-bordered table-striped mt-3 table2excel_with_colors">
        ...
        foreach ($data as $d)
        {
            ...
            <tr class="text-center <? if ($count == 1) {
                ...
                <td><?= (!empty($d->number_children)) ? $d->number_children : 0;
                    $child_total += $d->number_children; ?></td>
            </tr>
            <?
        }
        ?>
...
elseif (!empty($mun_org)) //по муниципальному
{
    ...
    {
        $mun_array = $user_model21->reportNew($organizations[0]['municipality_id'], 2, 2);
    }
    elseif ($post['title'] == 2020)
    {
        $mun_array = $user_model20->reportNew($organizations[0]['municipality_id'], 2, 2);
    }
...
elseif (!empty($reg_org)) //по региону
{
    if ($post['title'] == 2022)
    {
        $reg_array = $user_model->reportNew($municipalitys[0]['municipality_id'], 1, 3);
    }
    elseif($post['title'] == 2021)
    {
        $reg_array = $user_model21->reportNew($municipalitys[0]['region_id'], 2, 3);
    }
    elseif ($post['title'] == 2020)
    {
        $reg_array = $user_model20->reportNew($municipalitys[0]['region_id'], 2, 3);
    }
...
elseif (!empty($fed_org)) //по федеральному
{
    ...
    elseif ($post['title'] == 2021)
    {
        $fed_array = $user_model21->reportNew($regions[0]['district_id'], 2, 5);
    }
    elseif ($post['title'] == 2020)
    {
        $fed_array = $user_model20->reportNew($regions[0]['district_id'], 2, 5);
    }

    //print_r($fed_array);die();
    if (!isset($fed_array[0]) || !isset($fed_array[1]))
    {
        echo '<br><p class="text-center text-danger font-weight-bold">Обучение не начато ни в одном муниципальном районе!</p>';
    }
    else
    {
        ...
        <table id="tableId" class="table table-hover table-bordered table-striped mt-3 table2excel_with_colors">
            ...
            <tr class="text-center">
                ...
                <th rowspan="2" style="max-width: 180px;">Зарегистрировалось в программе</th>
                <th colspan="3" style="max-width: 180px;">Количество зарегистрировавшихся человек</th>
                <th colspan="12">Количество человек прошедших обучение</th>
                <th colspan="3">Завершили обучение</th>
                <th rowspan="2">Обучено детей</th>
            </tr class="text-center">
            <tr class="text-center">
            ...
            foreach ($regions as $region)
            {
                ...
                if (array_key_exists($region->id . '_training_id_1', $fed_array[0]) || array_key_exists($region->id . '_training_id_2', $fed_array[0])) //Количество зарегистрировавшихся человек (всего)
                {
                ...
                if (array_key_exists($region->id . '_training_id_2', $fed_array[0])) //Количество зарегистрировавшихся человек (взрослых)
                {
                ...
                if (array_key_exists($region->id . '_training_id_1', $fed_array[0])) //Количество зарегистрировавшихся человек (детей)
                {
                ...
                if (array_key_exists($region->id . '_theme6', $fed_array[1]))
                {
                    $theme6 = $fed_array[1][$region->id . '_theme6'];
                    $theme6_total += $theme6;
                }
                else
                {
                ...
                $trainingNotCompl = min($theme1, $theme2, $theme3, $theme4, $theme5, $independentWork) - $trainingCompletedAll;
                $trainingNotCompl_total += $trainingNotCompl;

                if (array_key_exists($region->id.'_numberChildren',$fed_array[1]))
                {
                    $childrenTrained = $fed_array[1][$region->id.'_numberChildren'];
                    $childrenTrained_total+=$childrenTrained;
                }
                else { $childrenTrained=0; }
                /*(END)*/
...
?>
    ...
    <script type="text/javascript">
        ...
        if (org === '1'){
            ...
            col17 = '<?php echo $trainingCompletedAll_total ?>';
            col18 = '<?php echo $trainingCompletedParent_total ?>';
            col19 = '<?php echo $trainingCompletedChild_total ?>';
            col20 = '<?php echo $childrenTrained_total ?>';
        }
        else if (fed_org === '1'){
            col1 = '<?php echo $startWork_total ?>';
            ...
            col5 = '<?php echo $people_total ?>';
            col6 = '<?php echo $parent_total ?>';
            col7 = '<?php echo $child_total ?>';
            col8 = '<?php echo ($inputTest_count == 0) ? 0 : round($inputTest_total / $inputTest_count, 1) ?>';
            col9 = '<?php echo $theme1_total ?>';
            col10 = '<?php echo $theme2_total ?>';
            col11 = '<?php echo $theme3_total ?>';
            col12 = '<?php echo $theme4_total ?>';
            col13 = '<?php echo $theme5_total ?>';
            col14 = '<?php echo $theme6_total ?>';
            col15 = '<?php echo $independentWork_total ?>';
            col16 = '<?php echo ($finalTest_count == 0) ? 0 : round($finalTest_total / $finalTest_count, 1) ?>';
            col17 = '<?php echo $finalTest_1st_total ?>';
            col18 = '<?php echo $finalTest_2st_total ?>';
            col19 = '<?php echo $trainingNotCompl_total ?>';
            col20 = '<?php echo $trainingCompletedAll_total ?>';
            col21 = '<?php echo $trainingCompletedParent_total ?>';
            col22 = '<?php echo $trainingCompletedChild_total ?>';
            col23 = '<?php echo $childrenTrained_total ?>';
        }
    </script>
