        ...
        <?foreach($menus_dishes as $key => $m_dish){ ?>
                ...
                <td class="text-center"><?= $m_dish->yield ?></td>
                    ...
                    <td class="text-center"><? $vitamins['f'] = round($m_dish->get_vitamin($m_dish->dishes_id, $m_dish->yield, 'f'),2); echo $vitamins['f']; $f = $f + $vitamins['f']?></td>
                        <td class="text-center"><? $vitamins['mg'] = round($m_dish->get_vitamin($m_dish->dishes_id, $m_dish->yield, 'mg'),2); echo $vitamins['mg']; $mg = $mg + $vitamins['mg']?></td>
                        <td class="text-center"><? $vitamins['p'] = round($m_dish->get_vitamin($m_dish->dishes_id, $m_dish->yield, 'p'),2); echo $vitamins['p']; $p = $p + $vitamins['p']?></td>
                        <td class="text-center"><? $vitamins['fe'] = round($m_dish->get_vitamin($m_dish->dishes_id, $m_dish->yield, 'fe'),2); echo $vitamins['fe']; $fe = $fe + $vitamins['fe']?></td>
                        <td class="text-center"><? $vitamins['i'] = round($m_dish->get_vitamin($m_dish->dishes_id, $m_dish->yield, 'i'),2); echo $vitamins['i']; $i = $i + $vitamins['i']?></td>
...
<!--                    <td class="text-center">--><?//= round($data[$nutrition->id]['vitamin_k']/$count_my_days, 3); $data_itog['vitamin_k'] = $data_itog['vitamin_k'] + round($data[$nutrition->id]['vitamin_k']/$count_my_days, 2);?><!--</td>-->
            ...
            <tr class="table-danger-2">
                ...
                <td class="text-center"><?=($data_itog['protein'] != 0) ? round($data_itog['fat']/$data_itog['protein'], 2) : 0;?></td>
                <td class="text-center"><?=($data_itog['protein'] != 0) ? round($data_itog['carbohydrates_total']/$data_itog['protein'], 2) : 0?></td>
            </tr>
            <tr class="table-danger-2">
                <td colspan="2">Процент от общей массы пищевых веществ	</td>
...
<?php } ?>
	...
	<br>
    <div class="text-center">
        <?= Html::a('<span class="glyphicon glyphicon-download"></span> Скачать техкарты блюд этого меню', ['export-techmup-this-menu-vihod?menu_id=' . $post['menu_id']],
            [
                'class'=>'btn btn-warning',
                'style' =>['width'=>'500px'],
                'title' => Yii::t('yii', 'Скачать отчет в формате PDF'),
	...
	<br>
    <div class="text-center">
        <?= Html::a('<span class="glyphicon glyphicon-download"></span> Скачать техкарты блюд по дням', ['export-techmup-this-menu-day-vihod?menu_id=' . $post['menu_id']],
            [
                'class'=>'btn btn-warning',
                'style' =>['width'=>'500px'],
                'title' => Yii::t('yii', 'Скачать отчет в формате PDF'),
